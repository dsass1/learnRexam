...
```{r Code}
question_text("Enter the exam code provided to verify you are in class. This must be submitted at the beginning of your exam. Failure to do so will result in an ungraded exam.",
              answer_fn(function(value){
                              if(str_remove_all(str_to_lower(value), " ") %in% c("20reg","stat20") ) {
                                return(mark_as(TRUE))
                                }
                              return(mark_as(FALSE) )
                              }),
...
exam(
  ...
  #Q1
  question_dropdown(
  "Consider a dataset of observations with a variable for time spent running and distance traveled. <br><br>
  ...
  question_dropdown(
    paste("Examine the graph below and then select the best estimate of the correlation. <br><br>", htmltools::img(src="images/cor_2.png", height = 200, width = 200)),
    answer("-0.70"),
  ...
  question_dropdown(
  paste("You run a simple linear regression model to predict student SAT math scores (`sat_math`) based on the state's percentage of seniors/juniors taking the SAT (pct_taking). The model fitted value and residual for 5 states were added to the dataset and are shown below. <br><br>", htmltools::img(src="images/lm_sat_3.png", height = 150, width = 500), "<br><br> Did the model over-, under-, or correctly estimate the mean SAT math score for OR (Oregon)?"),
  ...
  question_dropdown(
  "Using the simple linear regression model output above... <br><br> which of the five states does the model estimate worst?",
    answer("AZ"),
    answer("IL"),
  ...
  question_numeric(
  "Again using the simple linear regression model output above... <br><br> what is the fitted/predicted value for Illinois (`IL`)?",
  ...
  question_wordbank("Consider the following model where age and marital status (single, married) are the explanatory/predictor variables to predict household income. The reference group for marital status is **single**. <br><br>
  $$\\widehat{income} = b_0 + b_1(age) + b_2(marital status) + b_3(age*marital status)$$ <br>",
  ...
  question_dropdown(
  "You want to evaluate whether in-person classes or on-line classes provide better learning outcomes for STAT 202.  Students choose if and when they take STAT 202. From the students that choose to take the class you randomly assign half of the students to the in-person class and half the students to the on-line class and compare the test scores in each section. <br><br> What conclusions can we draw from this experiment?" ,
  ...
  question_wordbank("Northwestern is interested in understanding the quality of athletics at the school. <br><br> Identify the type of sampling method used:",
  ...
  question_radio(
  "To study job satisfaction of data scientists, a researcher obtained the email of all data scientists at the top 5 tech companies and emailed them a voluntary survey. The researcher computed an average job satisfaction level of 7 (on a scale of 1 to 10). <br> <br>
  ...
  question_dropdown(
  "Suppose you want to study if U.S. college students who prefer to drink beer, wine, and water differ in terms of their mean weights. Data were collected from a random sample of U.S. college students to address the research question above. The researchers found that college students who preferred beer tended to weigh more than those who preferred wine. <br> <br> Which one of the following interpretations is the most appropriate?",
    answer("College students who drink beer weigh more because they develop what is known as a beer belly."),
    answer("More college students should drink wine because it causes you to weigh less."),
    answer("Beer causes college students to weigh more because there were no confounding variables found in the dataset."),
    answer("There could be a confounding variable, that explains the difference in weight between beer and wine drinkers.", correct = TRUE),
...
A file named `survey.csv` is located in a `data/` subdirectory. Use the appropriate `readr` function to read in the dataset and store it as `survey`. There should not be any printed results.
...
Suppose a student has 750 Instagram followers (`followers_insta` = 750) and is an introvert (`personality` = `Introvert`). How many TikTok followers do we predict this student has?
...
-16.13971 +0.13548*750 -51.73801 +0.04944*750*1
```
...
question_wordbank(
  ...
  answer(c("don't_drink","don't_drink","beer"), correct = TRUE),
    allow_retry = TRUE,
    random_answer_order = TRUE,
    correct = "submitted",
    incorrect = "submitted"
...
Interpret the coefficient labeled **fav_drinkhard_liquor** in the **context of the problem**.
