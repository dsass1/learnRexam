...
sales_sample <- read_rds("data/sales_sample.rds")

# set.seed(459)
# sales_sample <- sales_data |>
#   sample_n(500) |>
#   select(-unit_price) |>
#   mutate(payment = ifelse(payment == "Ewallet", "Credit card", payment),
...
#write_rds(sales_sample, file = "sales_sample.rds")
...
Evaluate the claim that at least 45% of `Full Professor`s are `F`emale. 

Use the `salaries` data, `gender` and `title` variables to conduct a hypothesis test for the proportion of `Full Professor`s that are `F`emale at the $\alpha$ = 0.10 significance level. <br> <br> Are more than 45% of full professor's female??
