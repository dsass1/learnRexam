...
# write_rds(survey, "inst/tutorials/02_exam_a_23f/data/survey.rds")
...
exam(
  ...
  question_dropdown(
    paste("Examine the graph below and then select the best estimate of the correlation. <br>", htmltools::img(src="images/cor_1.png", height = 300, width = 300)),
    answer("-0.70"),
    answer("-0.30"),
    answer("0"),
  ...
  question_dropdown(
  paste("You run a simple linear regression model to predict student SAT math scores based on the state's percentage of seniors/juniors taking the SAT (pct_taking). The model fitted value and residual for 5 states are shown below. <br>", htmltools::img(src="images/lm_sat_1.png", height = 150, width = 500), "<br> Did the model over- or under-estimate the mean SAT math score for NV (Nevada)?"),
  ...
  question_dropdown(
  "Using the simple linear regression model output above... <br> Which of the five states does the model estimate best?",
  ...
  question_numeric(
  "Again using the simple linear regression model output above... <br> what is the fitted/predicted value for Kansas (`KS`)?",
  ...
  question_wordbank(
  "Consider the following model where age and gender are the explanatory/predictor variables, and **male** is the reference group for gender. <br>
  ...
  question_wordbank("Northwestern is interested in understanding the quality of athletics at the school. Identify the type of sampling method used:",
    choices = c("Northwestern randomly chooses 3 athletes from every one of its sports teams to participate in a survey",
                "Northwestern alphabatizes a list of all student athletes by last name and surveys the first 50 names."),
    box = 8,
...
29.18 + 0.557*65 +6.611*1 -0.009*65*1
```
